  ...
  (define (defines->let body)
    (map (lambda (exp)
           (if (definition? exp)
               ...
               exp))
         body))
  (let ((defines (body-defined body)))
    (if (null? defines)
        body
        (list (append (list 'let (name-unassigned (body-defined body)))
                      (defines->let body))))))
...
(define primitive-procedures
  (list (list 'car car)
        ...
        (list 'displayln (lambda (x) (display x) (newline)))
        (list 'modulo modulo)
        (list 'memq memq)
        (list 'member member)
        (list 'not not)
